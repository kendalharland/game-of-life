...
class SquareCellFactory implements CellFactory {
  ...
  List<Point> computeVertices(center, radius) => [
    new Point(center.x + (cos(PI/4) * radius).abs(), center.y - (sin(PI/4) * radius).abs()),
    new Point(center.x - (cos(3 * PI/4) * radius).abs(), center.y - (sin(3 * PI/4) * radius).abs()),
    new Point(center.x - (cos(5 * PI/4) * radius).abs(), center.y + (sin(5 * PI/4) * radius).abs()),
    new Point(center.x + (cos(7 * PI/4) * radius).abs(), center.y + (sin(7 * PI/4) * radius).abs()),
  ];
  ...
  static Point _computeCenter(x, y, radius) =>
    new Point(x, y) * 1.5 * radius;
}